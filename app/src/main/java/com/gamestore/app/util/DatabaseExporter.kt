        ...
        try {
            ...
            config.platforms.forEach { platform ->
                ...
                platform.databases.forEach { database ->
                    val databaseKey = "${platform.name}:${database.name}"
                    val dbGames = gamesMap[databaseKey] ?: emptyList()
                    val dbPath = database.path.substringAfter("databases/")
                    val dbFile = File(databasesDir, dbPath)
                    dbFile.writeText(gson.toJson(dbGames))
                    Log.d(TAG, "Created secondary database file for ${platform.name}:${database.name}: ${dbGames.size} games")
                }
            }
